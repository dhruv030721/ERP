    ...
    ({ label, type = "text", className = "", divclassName = "", labelclassName = "", placeholder="", ...props }, ref: ForwardedRef<HTMLInputElement>) => {
        ...
        return (
            <div className={`flex flex-col  ${divclassName}`}>
                ...
                <input type={type} placeholder={placeholder} id={id} className={`outline-none border py-3 px-2 rounded-lg border-gray-300 text-sm mt-3 w-full ${className}`} ref={ref} {...props} />
            </div>
        );
    }
);
